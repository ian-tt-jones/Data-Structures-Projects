...
class IQueue{
    ...
    Node* mergeRecur(Node* r1, Node* r2){
        ...
        if(m_heapType == MINHEAP){
            // making sure r1 is the smaller root
            if(m_priorFunc(r1 -> m_crop) > m_priorFunc(r2 -> m_crop)){
                Node* temp = r1;
                r1 = r2;
                r2 = temp;
            }
        ...
        // swap r1 left and right
        Node* temp = r1 -> m_left;
        r1 -> m_left = r1 -> m_right;
        r1 -> m_right = temp;

        // merge r2 and r1 -> m_left
        // set as r1 -> m_left
        r1 -> m_left = mergeRecur(r2, r1 -> m_left);

        return r1;
    }

    // used to deallocate all nodes
    void clearRecur(Node* n){
        if(n == nullptr){
            return;
        }

        clearRecur(n -> m_left);
        clearRecur(n -> m_right);
        delete n;
    }

    // recursive function used for a deep copy
    Node* copyRecur(Node* n){
        if(n == nullptr){
            return nullptr;
        }
        Node* newNode = new Node(n -> m_crop);
        newNode -> m_left = copyRecur(n -> m_left);
        newNode -> m_right = copyRecur(n -> m_right);
        m_size++;
        return newNode;
    }

    // used to print the crops of each node
    void printCropsQueueRecur(Node* n) const{
        if(n == nullptr){
            return;
        }

        cout << n -> m_crop << endl;
        printCropsQueueRecur(n -> m_left);
        printCropsQueueRecur(n -> m_right);
    }

    // used to reallocate nodes after a priority change
    void priorityShift(IQueue &iqueue, Node* n){
        if(n == nullptr){
            return;
        }

        iqueue.insertCrop(n -> m_crop);
        priorityShift(iqueue, n -> m_left);
        priorityShift(iqueue, n -> m_right);
    }
